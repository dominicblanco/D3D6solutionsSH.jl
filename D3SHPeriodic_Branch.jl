...
# The following code computes the solution and rigorously proves the results given in Theorems 4.5 and 4.6 of
...
#################################### List of the needed functions : go directly to line 421 for the main code ################################################# 
...
function Df(u,μ,γ)
    Df1 = LinearOperator(space(u),space(u),complex.(zeros(dimension(space(u)),dimension(space(u)))))
    Δ = project(Laplacian(2),space(u),space(u),Float64)
...
function DF(X,u̇,γ)
    Dfc1 = LinearOperator(space(X),space(X), complex.(zeros(dimension(space(X)),dimension(space(X)))))
    μ,u = eachcomponent(X)
    Dfc1 .= 0
...
function TangentVector(X,Ẋ,γ)
    ...
    component(Ẋn,2) .= u̇
    Ẋn = Ẋn/norm(Ẋn)
    @show Σ = real(sum(coefficients(component(Ẋn,2))'*coefficients(component(Ẋ,2))))
    if (Σ > 0)
        return Ẋn
    else
        return -Ẋn
...
function _newton_continue(ū,μ,Pfourier,arclength_grid,γ,Ẋ,npts)
    ...
    for k = 2:npts
        ...
        component(X,1)[1] = real(component(X,1)[1])
        J = component(X,2)
...
# Sometimes due to numerical error with D₃, the value of μ will become complex. 
# This is not what we expect as μ should be real, but since D₃ has coefficients in ℂ, 
# it is an unavoidable byproduct at times. To fix this, we use this function. 
# It is run after performing the continuation to ensure we have μ real on the branch.
function _make_real(X_grid,γ)
    l = length(X_grid)
    μ_grid2 = zeros(l)
    fourier = space(component(X_grid[1],2))
    X_grid2 = Vector{Sequence{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}}, Vector{ComplexF64}}}(undef,l)
    for k = 1:l 
        Vi,ϵ = _newton(component(X_grid[k],2),component(X_grid[k],1)[1],γ)
        X_grid2[k] = Sequence(ParameterSpace() × fourier, [component(X_grid[k],1)[1] ; coefficients(Vi)])
        μ_grid2[k] = component(X_grid2[k],1)[1]
    end
    return X_grid2,μ_grid2 
end
...
grid2cheb(x_fft, N) = 
    [RadiiPolynomial.ifft!(complex.(getindex.(x_fft, i)), Chebyshev(N)) for i ∈ indices(space(x_fft[1]))]
...
function F_V(X,Ẋ,γ,pfourier)
    l = length(X)
    F1 = Vector{Sequence{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}}, Vector{Complex{Interval{Float64}}}}}(undef,l)
    for i = 1:l
...
function _vec_to_seq(x,N,pfourier)
    l = length(x)
    y = Vector{Sequence{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}},Vector{Complex{Interval{Float64}}}}}(undef,N)
    for k = 1:l 
...
function _compute_tail(X_fft,γ,pfourier)
    l = length(X_fft)
    G_tail = Vector{Sequence{D₃Fourier,Vector{Complex{Interval{Float64}}}}}(undef,l)
    for i = 1:l 
...
function _compute_v_ϕ(X_fft,γ,pfourier,ν)
    ...
    v = Vector{Sequence{D₃Fourier,Vector{Complex{Interval{Float64}}}}}(undef,l)
    ϕ = Vector{Sequence{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}},Vector{Complex{Interval{Float64}}}}}(undef,l)
    for i = 1:l 
        Xi = Sequence(pfourier,X_fft[i])
        Vi = component(Xi,2)
...
function _mat_to_linop(M,pfourier)
    N = order(pfourier[2])[1]
    l = length(M)
    Mop = Vector{LinearOperator{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}},CartesianProduct{Tuple{ParameterSpace, D₃Fourier}},Matrix{Complex{Interval{Float64}}}}}(undef,l)
    for i = 1:l 
...
function _subtract_from_identity(Mop)
    l = length(Mop)
    Msub = Vector{LinearOperator{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}},CartesianProduct{Tuple{ParameterSpace, D₃Fourier}},Matrix{Complex{Interval{Float64}}}}}(undef,l)
    for i = 1:l 
...
function DFi(X,u̇,γ)
    Dfc1 = LinearOperator(space(X),space(X), complex.(interval.(zeros(dimension(space(X)),dimension(space(X))))))
    μ,u = eachcomponent(X)
    Dfc1 .= interval(0)
    component(Dfc1,1,2)[1,:] .= u̇
    project!(component(Dfc1,2,1),project(-u,space(u)))
    ...
    L_diag = interval(1) .+ interval.(diag(coefficients(Δ)))
    L = LinearOperator(space(u),space(u), Diagonal(L_diag.^2))
    U = project(Multiplication(u),space(u),space(u),Complex{Interval{Float64}})
    U² = project(Multiplication(u^2),space(u),space(u),Complex{Interval{Float64}})
...
function DF_V(X,Ẋ,γ,pfourier)
    l = length(X)
    DF1 = Vector{LinearOperator{CartesianProduct{Tuple{ParameterSpace, D₃Fourier}}, CartesianProduct{Tuple{ParameterSpace, D₃Fourier}}, Matrix{Complex{Interval{Float64}}}}}(undef,l)
    for i = 1:l 
        Xi = X[i]
        Ẋi = Ẋ[i]
        V̇i = component(Ẋi,2)
