...
# Note that this is the inequality of the norm, which is used in practice.
...
function _A_as_a_cheb_sequence_of_D6fourier_operators(A_cheb,K,pfourier)
    A_cheb0mat =  [real.(getindex.(A_cheb, i)) for i ∈ eachindex(coefficients(A_cheb[1])).-1] 
    A_cheb0 = Vector{LinearOperator{CartesianProduct{Tuple{ParameterSpace, D₆Fourier}}, CartesianProduct{Tuple{ParameterSpace, D₆Fourier}}, Matrix{Interval{Float64}}}}(undef,K+1)
    for i = 1:K+1 
...
function _X_as_a_cheb_sequence_of_D6fourier_sequences(X_cheb,K,pfourier)
    X_chebvec = [real.(getindex.(X_cheb, i)) for i ∈ eachindex(coefficients(X_cheb[1])).-1]
    X_cheb0 = Vector{Sequence{CartesianProduct{Tuple{ParameterSpace, D₆Fourier}},Vector{Interval{Float64}}}}(undef,K+1)
    for i = 1:K+1
...
function I_minus_ADF_as_a_cheb_sequence_of_D6fourier_operators(I_minus_ADF_cheb,K3,pfourier)
    I_minus_ADF_cheb_0mat =  [real.(getindex.(I_minus_ADF_cheb, i)) for i ∈ eachindex(coefficients(I_minus_ADF_cheb[1])).-1] 
    I_minus_ADF_cheb0 = Vector{LinearOperator{CartesianProduct{Tuple{ParameterSpace, D₆Fourier}}, CartesianProduct{Tuple{ParameterSpace, D₆Fourier}}, Matrix{Interval{Float64}}}}(undef,K3+1)
    N = order(pfourier[2])[1]
    for i = 1:K3+1 
...
G_tail_cheb = grid2chebonlyD6(complex.(G_tail_fft), K3)
...
@show Y₀
############################## The Bound Z₂ ############################
# The bound Z₂ as in Lemma 4.3.
B_cheb0 = _A_as_a_cheb_sequence_of_D6fourier_operators(B_cheb,K,pfourier)
w̄_cheb0 = _X_as_a_cheb_sequence_of_D6fourier_sequences(w̄_cheb,K,pfourier)
norm_B_cheb0 = opnorm_cheb(B_cheb0,ν)
fourier2 = D₆Fourier(2N,π/d)

q_cheb0 = interval(2)*γ .- interval(6)*component.(w̄_cheb0,2)
Z₂ = max(norm_B_cheb0,exact(2K+1)/L_N0)*(interval(1) + norm_cheb(q_cheb0,ℓ¹ν) + interval(3)*r₀)
@show Z₂
############################ The Bound Z₁ ##########################
...
v̄_cheb = grid2chebonlyD6(v̄_fft,K2)
...
Bϕ_cheb_K2 = [real.(getindex.(Bϕ_cheb_K2, i)) for i ∈ eachindex(coefficients(Bϕ_cheb_K2[1])).-1]
...
I_minus_BDF_cheb0 = I_minus_ADF_as_a_cheb_sequence_of_D6fourier_operators(I_minus_BDF_cheb,K2,pfourier)
